...
This repository contains the LaTeX source code, template files, and planning documents for the paper submitted to the **XIV Symposium on Knowledge Discovery, Mining and Learning (KDMiLe 2026)**.
...
*Difference is statistically significant according to McNemar's test ($\chi^2 = 74.40$, $p \approx 6.4 \times 10^{-18}$).*
...
```text
.
├── .gitignore                    # Git ignore rules for LaTeX build artifacts & local tools
...
To build the PDF of the paper locally, make sure you have a working LaTeX distribution installed (such as TeX Live) and run the following command in the root directory:
...
This compiles the document twice to resolve all cross-references (`\ref`) and outputs `urban-waste-yolo-vs-vlm.pdf`.
