...
\midrule
\emph{garbage-8uzha} & 1.838 & 1.838 & 0 \\
\emph{garbage-mvzg3} & 865 & 865 & 0 \\
\emph{Roadway} (fundo) & 271 & 0 & 271 \\
...
Para a inferência do Gemma 4, foram usados dois prompts: um para determinar se havia ao menos um saco de lixo visível e outro para localizar individualmente até 20 sacos, com caixas normalizadas no intervalo 0--1000. Resíduos avulsos foram explicitamente excluídos. Os textos completos estão em \texttt{prompt\_gemma\_classificacao.txt} e \texttt{prompt\_gemma\_localizacao.txt}; seus hashes SHA-256 são registrados automaticamente nos artefatos de cada execução.
...
Quanto à validade de construto, a tarefa foi reduzida à presença de sacos de lixo visíveis, sem estimar conteúdo, volume ou risco sanitário. A avaliação espacial por IoU 0,5 é diagnóstica, pois o VLM retorna até 20 caixas sem confiança. A anotação primária de cada imagem foi feita por um pesquisador e revisada pelo outro; o consenso reduz divergências, mas não produz uma medida independente de concordância interanotador.
