...
O teste foi construído com capturas do Google Street View de diferentes estados brasileiros, sem sobreposição com o treinamento. Cada pesquisador anotou 50\% das imagens e revisou os 50\% restantes, com divergências ajustadas por consenso; todos os sacos visíveis foram delimitados individualmente. O conjunto é balanceado (280 positivas e 280 negativas), tratando resíduos avulsos, lixeiras fechadas, vegetação e objetos utilitários como negativos.
...
Na identificação binária, o Gemma apresentou vantagem em todas as métricas, com diferença mais expressiva na precisão (apenas 1 falso positivo do VLM contra 47 do YOLO). Sua especificidade superior (0,9964 contra 0,8321; Tabela~\ref{tab:desempenho}) reflete a melhor rejeição de cenas sem o objeto-alvo. Em uma aplicação pública, falsos negativos representam sacos não identificados, enquanto falsos positivos geram custos de revisão.

Na localização, o padrão inverteu-se: das 785 caixas anotadas, o YOLO pareou 388 com $\mathrm{IoU} \ge 0,5$ contra apenas 127 do Gemma, que ainda gerou 858 caixas previstas (alto volume de falsos positivos espaciais). A vantagem do YOLO foi altamente significativa para sucesso por imagem ($p \approx 5,8 \times 10^{-27}$) e IoU médio ($p \approx 3,7 \times 10^{-30}$), consistente com a distinção entre reconhecimento global e localização regional \cite{Lin2024VLSAM}. A Figura~\ref{fig:iou-dist} evidencia essa separação: a distribuição do melhor IoU por imagem positiva concentra-se acima do limiar para o YOLO e abaixo dele para o Gemma. Assim, em resposta à \textbf{RQ1}, o Gemma supera na identificação binária e o YOLO na localização espacial.
...
\caption{Distribuição do melhor IoU por imagem positiva ($n = 280$). O YOLOv11m concentra-se acima de $\mathrm{IoU} = 0{,}5$ (localização precisa), enquanto o Gemma 4 31B-QAT acumula valores abaixo do limiar, explicando seu baixo F1 de caixas.}
\label{fig:iou-dist}
\end{figure}
...
O aumento do limiar reduz falsos positivos, mas eleva as omissões: em 0,65, a especificidade atinge 0,9429 e a sensibilidade cai para 0,7107. O ponto de operação deve, portanto, refletir o custo relativo dos erros: limiares menores favorecem triagens amplas com validação posterior, enquanto limiares maiores reduzem falsos alertas no acionamento automático, ao custo de mais ocorrências não identificadas.

\subsection{Eficiência computacional}
\label{sec:eficiencia-computacional}

A vantagem preditiva do Gemma foi acompanhada por custo computacional significativamente maior. O YOLOv11m operou com latência média \emph{end-to-end} de 10,2~ms (98,5~imagens/s), enquanto o Gemma 4 31B-QAT exigiu 1.278,6~ms por imagem (0,782~imagem/s) --- latência aproximadamente 126 vezes maior, dando ao YOLO vantagem decisiva em alto volume ou resposta próxima ao tempo real. A comparação representa estratégias distintas: o detector recebeu supervisão específica, enquanto o VLM se beneficia de pré-treinamento multimodal em maior escala \cite{Madan2024FewShotVLM}, e a implantação depende da distribuição da inferência entre dispositivo, borda e nuvem \cite{Magalhaes2019EdgeCloudYOLO}. Em resposta à \textbf{RQ2}, o Gemma oferece melhor identificação binária, enquanto o YOLO apresenta menor latência, maior throughput e localização espacial superior.


\subsection{Análise dos erros e complementaridade}
\label{sec:analise-erros}

Na classificação binária, os modelos discordaram em 79 imagens (68 acertadas só pelo Gemma e 11 só pelo YOLO): os falsos negativos do Gemma concentraram-se em cenas densas com múltiplos objetos similares, e os falsos positivos do YOLO, em calçadas escuras e sacos de entulho. Na localização, o padrão inverteu-se: o YOLO acertou em 130 imagens não localizadas pelo Gemma, contra apenas 11 no sentido oposto, com os falsos positivos espaciais do VLM concentrados em contêineres e sacolas de compras. A Figura~\ref{fig:caixas-tp-fp-fn} ilustra esses casos. A combinação de VLMs com componentes especializados de localização já foi explorada em detecção e segmentação aberta \cite{Kuo2023FVLM,Lin2024VLSAM}.
...
\caption{Exemplos qualitativos de localização no conjunto de teste. Caixas verdes indicam as anotações de referência (\emph{ground truth}); vermelhas, as previsões do YOLOv11m; azuis, as do Gemma 4 31B-QAT. O YOLO tende a alinhar-se às caixas de referência, enquanto o Gemma gera caixas em excesso e desalinhadas. Imagens originais \copyright~2026 Google. Fragmentos reproduzidos estritamente para fins acadêmicos e de pesquisa científica sob a doutrina de Uso Justo (\emph{Fair Use}).}
\label{fig:caixas-tp-fp-fn}
\end{figure}

Esses resultados sugerem uma cascata em que o VLM confirma a presença e o detector especializado fornece a localização precisa, mas o desempenho dessa arquitetura não é demonstrado pelos experimentos atuais.

\subsection{Implicações para a gestão pública}
\label{sec:implicacoes-gestao}

Os resultados sugerem três usos principais: o YOLOv11m em monitoramento contínuo, pela baixa latência e localização precisa; o Gemma 4 31B-QAT em auditorias pontuais orientadas à identificação binária, pela alta precisão e baixa taxa de falsos positivos; e uma arquitetura híbrida em triagens com confirmação. Em todos os cenários, a adoção deve incluir revisão humana, auditoria de decisões e reavaliação periódica.

\section{Conclusão}
\label{sec:conclusao}

Este trabalho comparou YOLOv11m e Gemma 4 31B-QAT na identificação e localização de sacos de lixo em 560 imagens de vias públicas brasileiras. O Gemma obteve melhor desempenho na identificação binária (F1 = 0,9650 versus 0,8685), enquanto o YOLOv11m, retreinado com o conjunto ampliado \emph{garbage-8uzha} (v4), demonstrou localização substancialmente superior (taxa de sucesso 0,8071 versus 0,3821 e F1 de caixas 0,5744 versus 0,1546). O YOLO foi também aproximadamente 126 vezes mais rápido em latência end-to-end.

Os resultados não sustentam a adoção universal de um único modelo: o Gemma favorece cenários orientados à qualidade preditiva na identificação, enquanto o YOLO é mais apropriado a triagens em larga escala e à localização precisa de instâncias. Trabalhos futuros devem ampliar a base municipal, testar modelos VLM menores, quantificar custos energéticos e avaliar sistemas híbridos que combinem a identificação do VLM com a localização do detector.

\noindent\textbf{Limitações.} A amostra de teste é balanceada artificialmente (50\% positivas), diferindo da prevalência esperada em monitoramento contínuo, e representa um subconjunto específico de cenas e enquadramentos brasileiros; os resultados constituem evidência comparativa no domínio avaliado. A tarefa restringe-se à presença visual de sacos, sem estimar volume ou risco sanitário, e a anotação por consenso entre dois pesquisadores não produz medida independente de concordância interanotador. A comparação reflete estratégias distintas (fine-tuning supervisionado versus zero-shot multimodal), e as medições de eficiência representam os fluxos implementados (Gemma via HTTP/LM Studio, YOLO local), sem experimentos de concorrência, energia ou custo em produção \cite{Madan2024FewShotVLM}.

\noindent\textbf{Disponibilidade de código e dados.} Os artefatos de reprodução e o notebook estão disponíveis em \url{https://github.com/jaimejrs/garbage-bag-yolov11m-vs-gemma4}. A publicação das imagens observará as licenças, os termos do Google Street View e os procedimentos de anonimização.
